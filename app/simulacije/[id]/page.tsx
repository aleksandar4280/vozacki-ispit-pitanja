    ...
    <div className="border rounded p-3 space-y-3">
      ...
      >
        <div>
          ...
          {q.mup_id ? (
            <div className="text-xs text-gray-500 mt-1">MUP ID: {q.mup_id}</div>
          ) : null}
        </div>
        {hasImage && (
...
export default function SimulationDetailPage() {
  ...
  const totalPoints = useMemo(
    () => filteredQs.reduce((s, q) => s + (q.points || 0), 0),
    [filteredQs]
  )
  ...
  async function saveEdit() {
    ...
    const clean = editQ.answers.map((a) => ({
      text: a.text.trim(),
      is_correct: !!a.is_correct,
    }))
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          >
            ...
            {areas.map((a) => (
              <option key={a.id} value={a.id}>
                {a.name} (
                {qs.filter((q) => subToArea.get(q.subarea_id) === a.id)
                  .length ?? 0}
                )
              </option>
            ))}
        ...
        <div>
          ...
          <select
            ...
            value={String(subId)}
            onChange={(e) =>
              setSubId(e.target.value ? Number(e.target.value) : '')
            }
            disabled={!areaId}
          >
        ...
        <div className="flex items-end">
          <div className="text-sm text-gray-700">
            {loading ? (
              ...
              <>
                Pronađeno: <b>{filteredQs.length}</b>
              </>
            )}
          </div>
        </div>
