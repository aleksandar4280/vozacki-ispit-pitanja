...
export default function NewSimulationPage() {
  ...
  const totalPoints = useMemo(
    () => picked.reduce((s, q) => s + (q.points || 0), 0),
    [picked]
  )
  ...
  async function save() {
    ...
    try {
      ...
      if (dupId) {
        alert(
          `Simulacija sa istim skupom pitanja već postoji (ID: ${dupId}). Nova nije sačuvana.`
        )
        return
      }
    } catch (e: any) {
        ...
        <div>
          <div className="flex items-center justify-between">
            <div className="text-sm font-medium">
              Odabrana pitanja ({count})
            </div>
            <div className="text-xs text-gray-600">
              Bodovi: {totalPoints}/100
            </div>
          </div>
          <div className="mt-2 space-y-2 max-h-[60vh] overflow-auto">
            {picked.map((q, idx) => (
              <div key={q.id} className="border rounded p-2">
                <div className="flex gap-2 items-start">
                  <div className="text-xs w-5 text-gray-600 mt-1">
                    {idx + 1}.
                  </div>
