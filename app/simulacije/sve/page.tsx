...
export default function AllSimulationQuestionsPage() {
  ...
  const [uniqOnSimsByArea, setUniqOnSimsByArea] = useState<Map<number, number>>(
    new Map()
  )
  const [uniqOnSimsBySub, setUniqOnSimsBySub] = useState<Map<number, number>>(
    new Map()
  )
    ...
    ;(async () => {
      ...
      try {
        ...
        const questionMap = new Map<
          number,
          { area_id: number | null; subarea_id: number | null }
        >()
        ...
        // 5) meta pitanja + odgovori samo za korišćena pitanja (+ mup_id)
        ...
        if (usedIds.length > 0) {
          ...
          for (const ids of chunk(usedIds, 1000)) {
            const { data } = await sb
              .from('questions')
              .select(
                'id,text,image_url,points,multi_correct,area_id,subarea_id,mup_id,answers(id,text,is_correct)' // ← DODATO mup_id
              )
              .in('id', ids)
            parts.push(...(data ?? []))
          }
  ...
  // frekvencija pojavljivanja po pitanju
  const freq = useMemo(() => {
    ...
    <AdminGuard>
      <div className="max-w-6xl mx-auto p-4 space-y-4">
        <h1 className="text-xl font-semibold">
          Sva pitanja sa svih simulacija (
          {new Set(pairs.map((p) => p.question_id)).size}/{pairs.length})
        </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setSubId(e.target.value ? Number(e.target.value) : 'all')
              }
            >
        ...
        <div className="space-y-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-600 mt-1 flex flex-wrap items-center gap-2">
                      ...
                      <span>
                        {correctCount === 1
                          ? 'Jedan tačan'
                          : formatTacni(correctCount)}
                      </span>
                      <span>•</span>
                      <span>
                        Ponavljanja: <b>{freq.get(q.id) ?? 0}</b>
                      </span>
                      {q.mup_id && (
                        <>
                          <span>•</span>
          ...
          {!loading && filtered.length === 0 && (
            <div className="text-gray-600">
              Nema pitanja za odabrani filter.
            </div>
          )}
        </div>
      </div>
