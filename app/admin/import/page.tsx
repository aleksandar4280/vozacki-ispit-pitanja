...
export default function ImportPage() {
  ...
  function tryParse() {
    ...
    try {
      ...
      j.forEach((q, i) => {
        ;['area', 'subarea', 'text', 'answers'].forEach((k) => {
          if (!(k in q)) throw new Error(`Red #${i + 1}: nedostaje ${k}`)
        })
        if (!Array.isArray(q.answers) || q.answers.length < 2)
          throw new Error(`Red #${i + 1}: potrebna su bar 2 odgovora.`)
      })
      setParsed(j)
    ...
    <AdminGuard>
      <div className="space-y-4">
        ...
        <p className="text-sm text-gray-700">
          Nalepi JSON (niz objekata):{' '}
          {
            '{ area, subarea, points, text, imageDataUrl?, answers:[{text,is_correct}] }'
          }
          .
        </p>
        ...
        <div className="flex gap-2">
          <button className="border rounded px-3 py-1" onClick={tryParse}>
            Proveri JSON
          </button>
          <button
                ...
                <li key={i} className="border rounded p-2">
                  ...
                  {q.imageDataUrl && (
                    <div className="mt-2">
                      <img
                        src={q.imageDataUrl}
                        alt="slika"
                        className="w-64 h-40 object-contain border rounded"
                      />
                    </div>
                  )}
                  <ul className="mt-2 text-sm list-disc list-inside">
                    {q.answers.map((a, j) => (
                      <li
                        key={j}
                        className={a.is_correct ? 'text-green-700' : ''}
                      >
