...
type TheoryLesson = { id: number; code: string }
...
export default function EditCandidatePage() {
  ...
  const [mupHits, setMupHits] = useState<
    {
      id: number
      text: string
      image_url: string | null
  ...
  // ✅ NOVO: Ručno čekiranje odslušanih časova
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const last =
        exRows && exRows.length > 0
          ? (exRows[0] as { exam_date: string; passed: boolean })
          : null
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setLessonsErr(null)
      try {
        const [{ data: tl, error: e1 }, { data: cl, error: e2 }] =
          await Promise.all([
            sb
              .from('theory_lessons')
              .select('id, code')
        ...
        setAllLessons((tl ?? []) as TheoryLesson[])
        const s = new Set<number>()
        ;(cl ?? []).forEach((r: any) => s.add(Number(r.lesson_id)))
        setListenedIds(s)
      } catch (e: any) {
        setLessonsErr(e?.message || 'Greška pri učitavanju časova.')
      } finally {
  ...
  async function save(e: React.FormEvent) {
    ...
    if (
      ...
      form.last_lesson_date &&
      new Date(form.last_lesson_date) < new Date(form.first_lesson_date)
    ) {
  ...
  async function saveNewExam() {
    ...
    if (selectedWrong.length > 0) {
      const rows = selectedWrong.map((w) => ({
        exam_id: ex.id,
        question_id: w.id,
      }))
  ...
  const pairedLessonCodes: Record<string, string> = {
    'T6-1': 'T8-1',
    'T8-1': 'T6-1',
    'T10-1': 'T11-1',
    'T11-1': 'T10-1',
  ...
  type LessonGroup = { key: string; ids: number[] }

  function buildLessonGroups(lessons: TheoryLesson[]): LessonGroup[] {
    const byCode = new Map<string, TheoryLesson>()
    lessons.forEach((l) => byCode.set(l.code, l))
    ...
    for (const l of lessons) {
      ...
      const pairCode = pairedLessonCodes[l.code]
      if (pairCode) {
        // kanonski key da ne dupliramo grupu
        const a = l.code
        const b = pairCode
        const key = [a, b].sort().join('+')

        const other = byCode.get(pairCode)
        const ids = other ? [l.id, other.id] : [l.id]

        groups.push({ key, ids })
        seen.add(a)
        seen.add(b)
      } else {
        groups.push({ key: l.code, ids: [l.id] })
        seen.add(l.code)
      }
    }
  ...
  function getPairedIds(lessonId: number): number[] {
    const current = allLessons.find((l) => l.id === lessonId)
    if (!current) return [lessonId]

    const pairCode = pairedLessonCodes[current.code]
    if (!pairCode) return [lessonId]

    const pair = allLessons.find((l) => l.code === pairCode)
    return pair ? [lessonId, pair.id] : [lessonId]
  }

  async function markAllLessons() {
    setLessonsErr(null)
    const ids = allLessons.map((l) => l.id)
    if (ids.length === 0) return

    // optimistic: čekiraj sve
    ...
    try {
      ...
      const { error } = await sb
        .from('candidate_lessons')
        .upsert(rows, { onConflict: 'candidate_id,lesson_id' })

      if (error) throw error
    } catch (e: any) {
      setLessonsErr(e?.message || 'Greška pri snimanju svih časova.')
      // rollback: vrati prazno (ili ako želiš, možeš refetch)
      setListenedIds(new Set())
    }
  }

  // ✅ helper: ručno čekiranje u candidate_lessons
  async function toggleLessonManual(lessonId: number) {
    const sb = supabaseBrowser()
    setLessonsErr(null)

    const ids = Array.from(new Set(getPairedIds(lessonId))) // [id] ili [id, pairId]
    const allChecked = ids.every((id) => listenedIds.has(id))
    ...
    try {
      ...
      setLessonsErr(e?.message || 'Greška pri snimanju časova.')

      // rollback
      setListenedIds((prev) => {
        const s = new Set(prev)
        for (const id of ids) {
          if (nextChecked) s.delete(id)
          else s.add(id)
        }
        return s
      })
  ...
  const lessonGroups = useMemo(
    () => buildLessonGroups(allLessons),
    [allLessons]
  )

  const listenedCount = useMemo(() => {
    // Spojeni čas = 1 ako je čekiran bar jedan ID iz te grupe
    // (normalno će biti oba čekirana, ali ovo je “tolerantno” ako je stanje nekad nedosledno)
    return lessonGroups.reduce((n, g) => {
      const any = g.ids.some((id) => listenedIds.has(id))
      return n + (any ? 1 : 0)
    }, 0)
  }, [lessonGroups, listenedIds])

  const totalClasses = lessonGroups.length // treba da ispadne 40
  ...
  if (!form)
    return (
      <AdminGuard>
        <p className="text-red-600">Kandidat nije pronađen.</p>
      </AdminGuard>
    )
      ...
      <div className="space-y-6">
        ...
        <form className="space-y-3" onSubmit={save}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <input
              className="border rounded p-2 w-full"
              placeholder="Telefon (npr. +381601234567)"
              value={form.phone ?? ''}
          ...
          <select
            ...
            onChange={(e) => update('school_id', Number(e.target.value))}
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  update('first_lesson_date', e.target.value || null)
                }
              />
            </div>
            <div>
              ...
              <input
                type="date"
                className="border rounded p-2 w-full"
                value={form.last_lesson_date ?? ''}
                onChange={(e) =>
                  update('last_lesson_date', e.target.value || null)
                }
              />
            </div>
          </div>
        ...
        {showExams && (
          <div className="bg-white rounded-xl shadow p-4 space-y-3">
            ...
            {!examsLoading && exams.length === 0 && (
              <p className="text-sm text-gray-600">Nema unetih ispita.</p>
            )}
              ...
              {exams.map((ex) => (
                <div key={ex.id} className="border rounded p-3">
                  <div className="flex flex-wrap items-center gap-3 justify-between">
                    ...
                    <div>
                      Poeni: <span className="font-medium">{ex.score}</span>
                    </div>
                    <div>
                      Status:{' '}
                      <span
                        className={`px-2 py-0.5 rounded text-xs ${ex.passed ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                      >
                  ...
                  {ex.wrong_questions && ex.wrong_questions.length > 0 && (
                    <ul className="mt-2 list-disc list-inside text-sm space-y-1">
                      {ex.wrong_questions.map((w) => (
                        <li key={w.question_id} className="break-words">
                          {w.questions?.text}
                        </li>
                      ))}
                    </ul>
        ...
        {newExamOpen && (
          <div
            ...
            onClick={() => setNewExamOpen(false)}
          >
            <div
              className="bg-white rounded-xl shadow p-4 w-full max-w-6xl max-h-[90vh] overflow-auto"
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <select
                    className="border rounded p-2 w-full"
                    value={examPassed}
                    onChange={(e) => setExamPassed(e.target.value as any)}
                  >
                    <option value="true">Položen</option>
                    <option value="false">Nije položen</option>
                  </select>
              ...
              <div className="mt-4">
                <button
                  className="border rounded px-3 py-1"
                  onClick={() => setWrongSearchOpen((v) => !v)}
                >
                  {wrongSearchOpen
                    ? 'Sakrij pogrešne odgovore'
                    : 'Pogrešni odgovori'}
                </button>
              </div>
                  ...
                  <div className="lg:col-span-3">
                    <div className="space-y-1">
                      ...
                      <div className="text-xs text-gray-600">
                        Unesi tačan ID za 1 rezultat ili deo ID-a za listu (max
                        50).
                      </div>
                    </div>
                    ...
                    {mupQ.trim() && (
                      <div className="mt-2 space-y-2">
                        <div className="text-sm text-gray-700">
                          {mupLoading
                            ? 'Pretražujem…'
                            : `Pronađeno: ${mupHits.length}`}
                          {mupErr && (
                            <span className="text-red-600 ml-2">{mupErr}</span>
                          )}
                        </div>
                          ...
                          {mupHits.map((q) => (
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  // već je dodato?
                                  if (selectedWrong.some((x) => x.id === q.id))
                                    return
                                  ...
                                  const { data, error } = await sb
                                    .from('questions')
                                    .select(
                                      'id, text, image_url, answers:answers(id, text, is_correct)'
                                    )
                                    .eq('id', q.id)
                                    .single()
                    ...
                    <div className="space-y-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <ul className="mt-2 text-sm space-y-1">
                                {q.answers.map((a) => (
                                  ...
                                  >
                                    <span
                                      className={`text-[10px] px-1.5 py-0.5 rounded mt-[2px] ${a.is_correct ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700'}`}
                                    >
                      ...
                      {searchRes.length === 0 && !searchLoading && (
                        <div className="text-sm text-gray-500">
                          Nema rezultata. Unesite deo teksta pitanja pa
                          pretražite.
                        </div>
                      )}
                    </div>
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="mt-1">
                                  <span className="text-xs text-gray-500">
                                    Odgovori: {q.answers.length}, tačnih:{' '}
                                    {
                                      q.answers.filter((a) => a.is_correct)
                                        .length
                                    }
                                  </span>
                                </div>
        ...
        {/* ✅ NOVO: Modal ručnog čekiranja časova */}
        {lessonsOpen && (
          <div
            className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
          ...
          >
            <div
              className="bg-white rounded-xl shadow p-4 w-full max-w-3xl max-h-[90vh] overflow-auto"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between gap-2">
                <h3 className="font-semibold">Odslušani časovi (ručno)</h3>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    ...
                    onClick={() => {
                      const ok = confirm(
                        'Da li ste sigurni? Ovo će označiti da je kandidat odslušao sve časove.'
                      )
                      if (ok) void markAllLessons()
                    }}
                    disabled={lessonsLoading || allLessons.length === 0}
                    title="Označi sve časove kao odslušane"
                  >
                    Odslušao sve
                  </button>
                  <button
              ...
              <div className="mt-2 text-sm text-gray-700">
                Odslušano: <b>{listenedCount}</b> / <b>{totalClasses}</b>
              </div>

              {lessonsErr && (
                <div className="mt-2 text-sm text-red-600">{lessonsErr}</div>
              )}
              {lessonsLoading && (
                <div className="mt-3 text-sm">Učitavanje…</div>
              )}

              {!lessonsLoading && (
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {allLessons.map((l) => {
                    const checked = listenedIds.has(l.id)
                    return (
                      <label
                        key={l.id}
                      ...
                      >
                        ...
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={() => toggleLessonManual(l.id)}
                        />
                      </label>
                    )
                  })}
                  {allLessons.length === 0 && (
                    <div className="text-sm text-gray-500">
                      Nema časova u bazi (theory_lessons).
                    </div>
                  )}
                </div>
              )}
