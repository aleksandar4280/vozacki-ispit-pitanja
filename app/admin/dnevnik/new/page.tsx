    ...
    <AdminGuard>
      <div className="max-w-xl mx-auto p-4 space-y-4">
        ...
        <div className="border rounded-xl p-4 bg-white space-y-3">
          <div>
            ...
            <div className="font-medium">
              {new Date(today + 'T00:00:00').toLocaleDateString('sr-RS')}
            </div>
          </div>
          ...
          <div className="text-sm text-gray-700">
            {selSchedule ? (
              <>
                Izabran raspored:{' '}
                <b>
                  {new Date(selSchedule.starts_at).toLocaleString('sr-RS', {
                    hour12: false,
                  })}
                </b>{' '}
                • {selSchedule.schools?.name ?? 'Autoškola'}
              </>
            ) : (
        ...
        {pickOpen && (
          ...
          >
            <div
              className="bg-white rounded-xl w-full max-w-lg max-h-[80vh] overflow-auto p-4"
              ...
              <div className="mt-3 space-y-2">
                {freeSchedules.map((s) => (
                  ...
                  >
                    <div>
                      <div className="font-medium">
                        {new Date(s.starts_at).toLocaleString('sr-RS', {
                          hour12: false,
                        })}
                      </div>
                      <div className="text-xs text-gray-600">
                        {s.schools?.name ?? 'Autoškola'}
                      </div>
                    </div>
                    <input
                      type="radio"
                ...
                {!loading && freeSchedules.length === 0 && (
                  <div className="text-sm text-gray-500">
                    Nema slobodnih rasporeda za današnji dan.
                  </div>
                )}
              </div>
            </div>
