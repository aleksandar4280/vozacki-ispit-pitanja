...
export default function NewSchedulePage() {
  ...
  const tableRows = useMemo(() => {
    const map = new Map<
      string,
      { firstIndex: number; lessonIds: number[]; codes: string[] }
    >()
    selectedIds.forEach((id, idx) => {
      ...
      if (!map.has(key))
        map.set(key, { firstIndex: idx, lessonIds: [], codes: [] })
  ...
  function toggleLesson(code: string) {
    ...
    if (willAdd) {
      ...
      newGroups.add(key)
      if (newGroups.size > 3) {
        setErr('Maksimalno 3 časa po rasporedu.')
  ...
  async function save() {
    ...
    const rows = selectedIds.map((lesson_id) => ({
    ...
    <AdminGuard>
      <div className="max-w-4xl mx-auto p-4 space-y-4">
          ...
          <div className="pt-2 border-t">
            ...
            <div className="mt-3 overflow-x-auto">
                    ...
                    return (
                      <tr key={row.key} className="text-sm">
                        ...
                        <td className="border px-2 py-1 text-center">
                          <button
                            className="rounded bg-red-100 hover:bg-red-200 px-2 py-1"
                            title="Obriši čas"
              ...
              <div className="text-xs text-gray-600 mt-2">
                Odabrano časova: <b>{tableRows.length}</b> / 3
              </div>
            </div>
        ...
        {pickOpen && (
          ...
          >
            <div
              className="bg-white rounded-2xl shadow w-full max-w-3xl max-h-[85vh] overflow-auto"
              ...
              <div className="p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {allLessons.map((l) => {
                  ...
                  return (
                    <label
                      key={l.id}
                      className={`border rounded-lg p-2 flex items-center gap-2 hover:bg-gray-50 ${checked ? 'bg-gray-50' : ''}`}
                    >
                      <input
                        type="checkbox"
                        checked={checked}
                        onChange={() => toggleLesson(l.code)}
                      />
                      <span className="text-sm">
                        {l.code}
                        {peer ? ' (par)' : ''}
                        {l.is_special ? ' • Posebni' : ''}
                      </span>
                    </label>
                  )
                })}
              </div>
              <div className="p-4 pt-0 text-xs text-gray-600">
                Parovi: <b>T6-1↔T8-1</b>, <b>T10-1↔T11-1</b>,{' '}
                <b>P-T2-1↔P-T9-1</b>.
              </div>
            </div>
          </div>
