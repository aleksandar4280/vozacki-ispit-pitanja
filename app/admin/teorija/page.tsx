...
export default function TheoryAdminPage() {
  ...
  const totalPlanned = useMemo(
    () =>
      themes.reduce(
        (s, x) => s + (Number.isFinite(x.lesson_count) ? x.lesson_count : 0),
        0
      ),
    [themes]
  )
  ...
  function updateCount(themeId: number, count: number) {
    setThemes((prev) =>
      prev.map((t) =>
        t.id === themeId
          ? { ...t, lesson_count: Number.isFinite(count) ? count : 0 }
          : t
      )
    )
  }
    ...
    <AdminGuard>
      <div className="max-w-4xl mx-auto p-4 space-y-4">
        <h1 className="text-xl font-semibold">
          Teorijska obuka – teme i časovi
        </h1>
        <p className="text-sm text-gray-600">
          Unesi broj časova za svaku temu (T1–T14), zatim klikni{' '}
          <b>Sačuvaj teme</b> i <b>Generiši časove</b>. Kodovi časova se
          automatski kreiraju (npr. T1-1, T1-2).
        </p>
          ...
          <>
            ...
            <div className="border rounded-xl p-3 bg-white shadow-sm space-y-2">
                ...
                {themes.map((t) => (
                  <div key={t.id} className="border rounded p-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateCount(t.id, Number(e.target.value))
                      }
                    />
                  </div>
                ))}
              ...
              <div className="text-sm text-gray-700">
                Planirano ukupno časova: <b>{totalPlanned}</b>{' '}
                {totalPlanned !== 40 && '(napomena: očekivano 40)'}
              </div>
