...
function normalizeAnswers(inAnswers: IncomingAnswer[]) {
  ...
  if (!out.some((x) => x.is_correct))
    throw new Error('Bar jedan odgovor mora biti tačan.')
  return out
}
  ...
  try {
    ...
    for (let idx = 0; idx < items.length; idx++) {
      ...
      try {
        ...
        const points = Number.isFinite(item.points as any)
          ? Number(item.points)
          : 1

        if (!areaName || !subName || !qText)
          throw new Error('Nedostaju area/subarea/text.')

        const answers = normalizeAnswers(item.answers)

        // Upsert area
        let { data: area } = await supa
          .from('areas')
          .select('id')
          .eq('name', areaName)
          .maybeSingle()
        ...
        if (!sub) {
          const ins = await supa
            .from('subareas')
            .insert({ area_id: area.id, name: subName })
            .select('id')
            .single()
          if (ins.error) throw ins.error
          sub = ins.data
        ...
        errors.push({
          ...
          text: items[idx]?.text?.slice(0, 120),
        })
      }
    }
