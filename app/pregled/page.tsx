...
export default function PregledPage() {
  ...
  const filteredSubs = useMemo(
    () =>
      subs.filter(
        (s) => s.area_id === (typeof areaId === 'number' ? areaId : -1)
      ),
    [subs, areaId]
  )
  ...
  function addAnswerRow() {
    ...
    setEditQ({
      ...editQ,
      answers: [...editQ.answers, { text: '', is_correct: false }],
    })
  }
          ...
          <div>
            ...
            >
              ...
              {subs
                .filter(
                  (s) =>
                    s.area_id === (typeof areaId === 'number' ? areaId : -1)
                )
                .map((s) => (
        ...
        <div className="space-y-4">
          {questions.map((q) => (
            <QuestionCard
              ...
              onToggleReveal={() =>
                setRevealBySub((prev) => ({ ...prev, [q.id]: !prev[q.id] }))
              }
          ...
          {loaded && questions.length === 0 && (
            <div className="text-gray-600">
              Nema pitanja za izabranu podoblast.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="space-y-1">
          <label className="block text-sm">
            Pretraga po tekstu pitanja (globalno – sve oblasti i podoblasti)
          </label>
        </div>
        <input
          className="w-full border rounded p-2"
        ...
        <div className="text-sm text-gray-700">
          {globalLoading ? (
            ...
            <>
              Pronađeno: <b>{globalResults.length}</b>
            </>
          ) : (
            'Unesi bar 2 znaka za globalnu pretragu.'
          )}
        ...
        <div className="space-y-4">
          {globalResults.map((q) => (
            <QuestionCard
              ...
              onToggleReveal={() =>
                setRevealGlobal((prev) => ({ ...prev, [q.id]: !prev[q.id] }))
              }
              onPreview={(u) => setPreviewUrl(u)}
              onEdit={() => openEdit(q)} // ✅
            />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  value={editQ.mup_id}
                  onChange={(e) =>
                    setEditQ({ ...editQ, mup_id: e.target.value })
                  }
              ...
              <div>
                ...
                <select
                  className="w-full border rounded p-2"
                  value={String(
                    subs.find((s) => s.id === editQ.subarea_id)?.area_id ?? ''
                  )}
                  onChange={(e) => {
                    ...
                    setEditQ({
                      ...editQ,
                      subarea_id: firstSub ? firstSub.id : null,
                    })
                  }}
                >
                  <option value="">Izaberi oblast</option>
              ...
              <div className="md:col-span-2">
                ...
                <div className="text-xs text-gray-600 mt-1">
                  Napomena: pitanje mora imati ≥ 2 odgovora; označi jedan ili
                  više tačnih.
                </div>
              </div>
            </div>
