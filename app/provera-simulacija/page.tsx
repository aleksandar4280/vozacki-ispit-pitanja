...
export default function ProveraSimulacijaPage() {
  ...
  const simSets = useMemo(() => {
    ...
    for (const row of simPairs) {
      if (!map.has(row.simulation_id))
        map.set(row.simulation_id, new Set<number>())
  ...
  useEffect(() => {
    ...
    for (const exam of exams) {
      const wrongIds = (exam.wrong_questions ?? [])
        .map((w) => w.question_id)
        .filter(Boolean)
    ...
    <div className="max-w-5xl mx-auto p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">
          Provera poklapanja sa simulacijama
        </h1>
      </div>
      ...
      <div className="text-sm text-gray-700">
        {loading ? (
          ...
          <>
            Pronađenih poklapanja: <b>{matches.length}</b>
          </>
        )}
        ...
        <div className="divide-y">
          {matches.map((m) => (
            ...
            >
              ...
              <div className="col-span-3">
                <Link
                  href={`/simulacije/${m.simulation.id}`}
                  className="underline"
                >
                  Simulacija {m.simulation.id}
                  {m.simulation.title ? ` – ${m.simulation.title}` : ''}
                </Link>
              </div>
            </div>
          ))}
