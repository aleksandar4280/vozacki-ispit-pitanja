...
if (!SUPABASE_URL) {
  console.error(
    'Greška: SUPABASE_URL nije setovan (ni NEXT_PUBLIC_SUPABASE_URL). Proveri .env.local/.env'
  )
  process.exit(1)
}
if (!SERVICE_KEY) {
  console.error(
    'Greška: SERVICE KEY nije setovan. Postavi SUPABASE_SERVICE_ROLE_KEY (ili bar NEXT_PUBLIC_SUPABASE_ANON_KEY za public bucket).'
  )
  process.exit(1)
}
